...
#define nrn_init _nrn_init__NetStim_priority
#define _nrn_initial _nrn_initial__NetStim_priority
#define nrn_cur _nrn_cur__NetStim_priority
#define _nrn_current _nrn_current__NetStim_priority
#define nrn_jacob _nrn_jacob__NetStim_priority
#define nrn_state _nrn_state__NetStim_priority
#define _net_receive _net_receive__NetStim_priority 
#define init_sequence_w init_sequence_w__NetStim_priority 
#define init_sequence init_sequence__NetStim_priority 
#define next_invl next_invl__NetStim_priority 
#define noiseFromRandom noiseFromRandom__NetStim_priority 
#define seed seed__NetStim_priority 
...
extern "C" {
 ...
 static double _hoc_init_sequence_w();
...
extern Memb_func* memb_func;
 ...
 "erand", _hoc_erand,
 "init_sequence_w", _hoc_init_sequence_w,
...
#define erand erand_NetStim_priority
#define invl invl_NetStim_priority
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 NetStim_priority /home/neuron/projects/rete/control_sims/weasel/templates/granule_multicomp/x86_64/NetStim_priority.mod\n");
...
static int init_sequence_w(_threadargsprotocomma_ double);
static int init_sequence(_threadargsprotocomma_ double, double);
...
static int  init_sequence ( _threadargsprotocomma_ double _lt , double _lw ) {
   if ( number > 0.0 ) {
     on = 1.0 ;
     event = 0.0 ;
     ispike = 0.0 ;
...
static double _hoc_init_sequence(void* _vptr) {
 double _r;
  ...
  _nt = (_NrnThread*)((Point_process*)_vptr)->_vnt;
 _r = 1.;
 init_sequence ( _p, _ppvar, _thread, _nt, *getarg(1) , *getarg(2) );
 return(_r);
}
 
static int  init_sequence_w ( _threadargsprotocomma_ double _lt ) {
   if ( count_down_spikes > 0.0 ) {
     on = 1.0 ;
     event = 0.0 ;
     ispike = count_down_spikes ;
     }
    return 0; }
 
static double _hoc_init_sequence_w(void* _vptr) {
 double _r;
   double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
   _p = ((Point_process*)_vptr)->_prop->param;
  _ppvar = ((Point_process*)_vptr)->_prop->dparam;
  ...
  _nt = (_NrnThread*)((Point_process*)_vptr)->_vnt;
 _r = 1.;
 init_sequence_w ( _p, _ppvar, _thread, _nt, *getarg(1) );
 return(_r);
}
 ...
 _tsav = t;   if (_lflag == 1. ) {*(_tqitem) = 0;}
 {
   if ( _lflag  == 0.0 ) {
     ...
     if ( _args[0]  == 0.0  && on  == 0.0 ) {
       ...
       if ( ispike  == 0.0 ) {
         printf ( "init\n" ) ;
         init_sequence_w ( _threadargscomma_ t ) ;
         }
       ispike = ispike - 1.0 ;
       printf ( "ispike %g\n" , ispike ) ;
       if ( ispike  == 0.0 ) {
