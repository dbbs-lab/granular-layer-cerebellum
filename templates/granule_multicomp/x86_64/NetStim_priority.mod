...
NEURON	{ 
  ARTIFICIAL_CELL NetStim_priority
  RANGE interval, number, start
  RANGE noise, count_down_spikes
...
PROCEDURE init_sequence(t(ms),w) {
    if (number > 0) {
	on = 1
	event = 0
	ispike = 0
...
PROCEDURE init_sequence_w(t(ms)) {
    if (count_down_spikes > 0) {
	on = 1
	event = 0
...
NET_RECEIVE (w) {
    if (flag == 0) { : external event
		...
		if (w == 0 && on == 0) { : count down for end of spiking pause
		    printf("zeroweigth\n")
		    if (ispike == 0) {
			printf("init\n")
			init_sequence_w(t)
		    }
		    : receiving a 0 weight spike means wait for the other 
		    : net stim to finish spiking
		    : the number of the two net stims must be equal
		    ispike = ispike - 1
		    printf("ispike %g\n",ispike)
		    if (ispike == 0) {
			on = 1
