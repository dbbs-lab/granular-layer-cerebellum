...
#define nrn_init _nrn_init__GRANULE_Ampa_det_Prel
#define _nrn_initial _nrn_initial__GRANULE_Ampa_det_Prel
#define nrn_cur _nrn_cur__GRANULE_Ampa_det_Prel
#define _nrn_current _nrn_current__GRANULE_Ampa_det_Prel
#define nrn_jacob _nrn_jacob__GRANULE_Ampa_det_Prel
#define nrn_state _nrn_state__GRANULE_Ampa_det_Prel
#define _net_receive _net_receive__GRANULE_Ampa_det_Prel 
#define kstates kstates__GRANULE_Ampa_det_Prel 
...
#define D _p[10034]
#define x _p[10035]
#define tsyn _p[10036]
#define Mres _p[10037]
#define numpulses _p[10038]
#define tzero _p[10039]
#define gbar_Q10 _p[10040]
#define Q10 _p[10041]
#define DC _p[10042]
#define DO _p[10043]
#define DD _p[10044]
...
#define diffusione diffusione_GRANULE_Ampa_det_Prel
 extern double diffusione( _threadargsproto_ );
 /* declare global and static user variables */
#define u0 u0_GRANULE_Ampa_det_Prel
 double u0 = 0;
 ...
 "tau_rec", 1e-09, 1e+09,
 "tau_1", 1e-09, 1e+09,
 "u0_GRANULE_Ampa_det_Prel", 0, 1,
 0,0,0
};
 static HocParmUnits _hoc_parm_units[] = {
 "u0_GRANULE_Ampa_det_Prel", "1",
 "gmax", "pS",
 "U", "1",
 "tau_rec", "ms",
 "tau_facil", "ms",
 ...
 "u0_GRANULE_Ampa_det_Prel", &u0_GRANULE_Ampa_det_Prel,
 0,0
};
...
static void nrn_alloc(Prop* _prop) {
  ...
  if (nrn_point_prop_) {
	...
	_ppvar = nrn_point_prop_->dparam;
 	...
 	_p = nrn_prop_data_alloc(_mechtype, 10048, _prop);
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 void _GRANULE_Ampa_det_Prel_reg() {
	int _vectorized = 1;
  _initlists();
...
#endif
  hoc_register_prop_size(_mechtype, 10048, 4);
 ...
 pnt_receive_size[_mechtype] = 4;
 	...
 	ivoc_help("help ?1 GRANULE_Ampa_det_Prel /home/stefano/Documenti/paperRevision/GL_Model/x86_64/GRANULE_Ampa_det_Prel.mod\n");
...
double diffusione ( _threadargsproto_ ) {
   double _ldiffusione;
 double _lDifWave , _li , _laaa ;
 _lDifWave = 0.0 ;
   {int  _li ;for ( _li = 1 ; _li <= ((int) numpulses ) ; _li ++ ) {
     tzero = tspike [ _li - 1 ] ;
     if ( t > tzero ) {
       _laaa = ( - Rd * Rd / ( 4.0 * Diff * ( t - tzero ) ) ) ;
       if ( fabs ( _laaa ) < 699.0 ) {
         _lDifWave = _lDifWave + Mres * exp ( _laaa ) / ( ( 4.0 * PI * Diff * ( 1e-3 ) * lamd ) * ( t - tzero ) ) ;
         }
       else {
         if ( _laaa > 0.0 ) {
           _lDifWave = _lDifWave + Mres * exp ( 699.0 ) / ( ( 4.0 * PI * Diff * ( 1e-3 ) * lamd ) * ( t - tzero ) ) ;
           }
         else {
           _lDifWave = _lDifWave + Mres * exp ( - 699.0 ) / ( ( 4.0 * PI * Diff * ( 1e-3 ) * lamd ) * ( t - tzero ) ) ;
           }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  O = O0;
 {
   ...
   numpulses = 0.0 ;
   {int  _li ;for ( _li = 1 ; _li <= 5000 ; _li ++ ) {
     PRE [ _li - 1 ] = 0.0 ;
     tspike [ _li - 1 ] = 0.0 ;
     } }
   tspike [ 0 ] = 1e12 ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/stefano/Documenti/paperRevision/GL_Model/templates/synapse/GRANULE_Ampa_det_Prel.mod";
static const char* nmodl_file_text = 
  "TITLE \n"
  "\n"
  "COMMENT\n"
  ...
  "		if(fabs(aaa)<699){\n"
  "		    DifWave=DifWave+Mres*exp(aaa)/((4*PI*Diff*(1e-3)*lamd)*(t-tzero)) : ^nd nd =1\n"
  "		}else{\n"
  "		    if(aaa>0){\n"
  "			DifWave=DifWave+Mres*exp(699)/((4*PI*Diff*(1e-3)*lamd)*(t-tzero)) : ^nd nd =1\n"
  "		    }else{\n"
  "			DifWave=DifWave+Mres*exp(-699)/((4*PI*Diff*(1e-3)*lamd)*(t-tzero)) : ^nd nd =1\n"
  "		    }\n"
  "		}\n"
